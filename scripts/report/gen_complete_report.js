  ...
  sections: [
    ...
    { properties:{page:{size:{width:11906,height:16838},margin:{top:1418,right:1134,bottom:1134,left:1418}}},
      ...
      children:[
        ...
        CL("    pstmt.setString(3, emp.getEmpNo());"),
        CL("    pstmt.setString(4, emp.getEmpAddr());"),
        CL("    pstmt.setDouble(5, emp.getEmpSalary());"),
        ...
        CL("}"),
        E(),
        P([R("使用?占位符替代字符串拼接是JDBC编程的核心安全实践。"),R("若采用字符串拼接方式构造SQL（如\"'\" + name + \"'\")，当用户输入包含单引号时（例如O'Brien)，引号会提前闭合字符串导致SQL语法错误。更为严重的安全威胁是SQL注入攻击——攻击者通过输入\"' OR '1'='1\"等精心构造的字符串，可以绕过查询条件限制，获取未授权的数据访问权限。PreparedStatement将SQL骨架与参数值分离传递，由数据库驱动对参数进行自动转义处理，从机制层面杜绝了注入攻击的可能性。代码中SQL字符串过长时使用+运算符换行拼接的方式，仅在编译阶段合并为一个完整字符串，不影响SQL语义和数据库执行计划。")]),
        ...
        E(),
        P([R("Emp.java —— 实体类。"),R("Lombok是Java开发中的效率利器。@Data自动生成所有字段的getter/setter/toString/equals/hashCode，@NoArgsConstructor生成无参构造器，@AllArgsConstructor生成全参构造器。@Data已经包含了@ToString，为什么还要单独写一个？这不是多余——万一日后@Data实现变化，显式声明的@ToString能确保toString始终存在。另外empSalary使用Double而非double，因为数据库薪资字段可能为NULL，而基本类型double无法表示null：")]),
        ...
        CL("    }"),
        CL(""),
        CL("    public List<Emp> findAll() throws Exception {"),
        CL("        conn = DBUtil.getConnection();"),
        CL("        String sql = \"select * from emp\";"),
        CL("        PreparedStatement pstmt = conn.prepareStatement(sql);"),
        CL("        ResultSet rs = pstmt.executeQuery();"),
        CL("        List<Emp> list = new ArrayList<>();"),
        CL("        while (rs.next()) {"),
        CL("            Emp e = new Emp();"),
        CL("            e.setEmpId(rs.getInt(1));"),
        CL("            e.setEmpName(rs.getString(2));"),
        CL("            e.setEmpTel(rs.getString(3));"),
        CL("            e.setEmpNo(rs.getString(4));"),
        CL("            e.setEmpAddr(rs.getString(5));"),
        CL("            e.setEmpSalary(rs.getDouble(6));"),
        CL("            e.setTypeId(rs.getInt(7));"),
        CL("            list.add(e);"),
        CL("        }"),
        CL("        DBUtil.close(conn);"),
        CL("        return list;"),
        CL("    }"),
        CL(""),
        CL("    public int getEmpCount() throws Exception {"),
        CL("        conn = DBUtil.getConnection();"),
        CL("        String sql = \"select count(*) from emp\";"),
        CL("        PreparedStatement pstmt = conn.prepareStatement(sql);"),
        CL("        ResultSet rs = pstmt.executeQuery();"),
        CL("        if (rs.next()) {"),
        CL("            int count = rs.getInt(1);"),
        CL("            DBUtil.close(conn);"),
        CL("            return count;"),
        CL("        }"),
        CL("        DBUtil.close(conn);"),
...
// ══════════ 输出 ══════════
const out = "E:\\我的桌面\\202532044314-马康博\\202532044314-马康博.docx";
